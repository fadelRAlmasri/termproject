...
<html>
    <head>
        <title>اضافة  تفاصيل فاتورة شراء
        </title>
        <meta charset="utf-8">
...
if(isset($_POST['send'])){
      $num=$_POST['num'];
      $name=$_POST['name'];
      $a= "SELECT id from baybill where num='{$num}'";
      ...
      $amount=$_POST['amount'];
      $note=$_POST['note'];
    $sql=$con->prepare("INSERT INTO baybilldetails (billid,	subid,	amount,	note)VALUES ('".$r3['id']."','".$r4['id']."','$amount','$note') ");
    if($sql->execute()){
        echo "";
    }    
}
